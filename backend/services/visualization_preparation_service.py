...
def _safe_float(v) -> float:
    """Convert a value to float, returning 0 for NaN/Inf."""
    try:
        f = float(v)
...
def _aggregate_data(
    ...
    COUNT is the only aggregation that works on any column type.
    All other aggregations require a numeric y_col; falls back to COUNT if the
    column produces no valid numeric values after coercion.
    """
    # ── No y_col: always count rows ──────────────────────────────────────────
    if y_col is None or y_col not in df.columns:
        result = df.groupby(x_col, observed=True).size().reset_index(name="_value_")
        return result

    # ── COUNT: count non-null y values per group (works on any dtype) ────────
    if agg == "count":
        tmp = df[[x_col, y_col]].copy()
        # replace empty strings with NA so they are not counted
        tmp[y_col] = tmp[y_col].replace("", pd.NA)
        result = tmp.groupby(x_col, observed=True)[y_col].count().reset_index()
        result.columns = [x_col, "_value_"]
        return result

    # ── Numeric aggregations: coerce y_col to numbers, drop non-numeric ──────
    # Pandas agg function name for each user-facing label
    _AGG_MAP = {
        "sum":    "sum",
        "avg":    "mean",
        "min":    "min",
    ...
    pandas_agg = _AGG_MAP.get(agg, "sum")
    ...
    if tmp.empty:
        # y_col has no valid numeric values — fall back to row count
        result = df.groupby(x_col, observed=True).size().reset_index(name="_value_")
        return result

    result = tmp.groupby(x_col, observed=True)[y_col].agg(pandas_agg).reset_index()
    result.columns = [x_col, "_value_"]
    return result
...
class VisualizationPreparationService:
    ...
    Supports axis-aware rendering with full aggregation pipeline.
    Pipeline order: Aggregate → Render
    """
    ...
    def _scatter_xy(self, df: pd.DataFrame, xcol: str, ycol: str) -> dict:
        ...
        points = [{"x": _safe_float(r[xcol]), "y": _safe_float(r[ycol])} for _, r in plot_df.iterrows()]
    ...
    def _line_area_xy(
        ...
        agg_func = _LINE_AGG_MAP.get(aggregation, "sum")
    ...
    def _bar_xy(
        ...
        # Cap to max categories for performance
        if len(agg_df) > self.max_categories_bar:
            agg_df = agg_df.head(self.max_categories_bar).reset_index(drop=True)

        labels = [_truncate_label(l) for l in agg_df[xcol].tolist()]
        data = [_safe_float(v) for v in agg_df[value_col].tolist()]
    ...
    def _pie_donut_xy(
        ...
        # Cap to max pie categories for readability
        if len(agg_df) > self.max_categories_pie:
            agg_df = agg_df.head(self.max_categories_pie).reset_index(drop=True)
